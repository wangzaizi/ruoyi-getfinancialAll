...
**问题**：之前要求最终HTML的URL本身必须包含"2024"、"决算"、"市级"等关键词，但实际很多符合条件的页面URL并不包含这些关键词。

**解决方案**：
- 不再检查URL本身是否包含关键词
- 改为检查**链接元素文本**和**目标页面内容**（`<title>`、`<h1>`等）
- 验证逻辑：同时包含目标年份（2024）、"决算"、本级标识（市级/本级/城市名）即可通过
- 仅对HTML页面生效（文件直链不在此判断范围）

**实现位置**：`spider.py` 中的 `_is_final_decision_html` 方法

### 2. 早停策略

**问题**：找到符合条件的HTML页面后，仍继续搜索其他页面，浪费时间和资源。

**解决方案**：
- 一旦在公开栏目路径下找到符合条件的HTML页面，立即返回该结果
- 文件直链不触发早停（因为HTML页面才是目标页面）
- 大幅提升搜索效率，减少不必要的网络请求

**实现位置**：`spider.py` 中的 `search_finance_reports` 方法

### 3. 公开栏目关键词扩展

**问题**：不同城市的政府网站使用不同的栏目命名，原有关键词可能无法覆盖所有情况。
...
- 扩充公开栏目关键词集合，新增：
  ...
  - `政府决算公开`、`财政决算公开`
  - `市政府财政预决算`、`三公` 等
- 根据实际路径示例（如"政府信息公开>法定主动公开内容>财政资金>政府决算公开"）优化关键词匹配

**实现位置**：`spider.py` 中的 `search_finance_reports` 方法

### 4. 暴力遍历优化

**问题**：暴力遍历时没有优先级，可能探索不相关的链接，效率低下。

**解决方案**：
- **关键词评分机制**：对候选链接进行评分，优先探索包含相关关键词的链接
  - 评分依据：链接标题和URL中包含"法定主动公开内容"、"财政预决算"、"政府决算公开"等关键词的数量
  - 评分高的链接优先入队，提高命中率
- **深度限制增加**：从2层增加到3层，适配更多实际路径层级
- **页数上限增加**：从120页增加到180页，提高覆盖率

**实现位置**：`spider.py` 中的 `search_finance_reports` 方法的暴力遍历部分

### 5. 增量爬取优化（`crawl_city_public_urls.py`）

**问题**：每次重新运行都会爬取所有城市，浪费时间和资源；已成功的城市数据可能被覆盖。

**解决方案**：
- **读取现有结果**：自动加载 `results.json` 文件
- **只处理失败城市**：仅对 `success: false` 的城市重新爬取
- **使用暴力遍历**：对失败城市使用 `violent_fallback=True` 进行深度搜索
- **保留成功数据**：`success: true` 的城市数据保持不变，不会被覆盖
- **增量更新**：只更新失败城市的结果，保留成功城市的数据

**工作流程**：
1. 加载现有的 `results.json`
2. 筛选出 `success: false` 的城市
3. 对这些城市使用暴力遍历模式重新爬取
4. 合并结果：
   - 保留所有 `success: true` 的城市（原样）
   - 用新结果更新失败的城市
5. 更新 `results.json` 文件

**使用场景**：
- 首次运行：爬取所有城市
- 后续运行：只重新爬取失败的城市，使用更激进的搜索策略
- 结果文件自动更新，保留已成功的数据
